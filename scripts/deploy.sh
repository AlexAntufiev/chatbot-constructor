...
printf "POSTGRES_USER=${db_user}\nPOSTGRES_PASSWORD=${db_password}" > ./docker/db.env

time ssh -i ${ssh_path} ${server_user}@${PROD_IP} "mkdir -p ${app_name}"
rsync -avz -e "ssh -i ${ssh_path}" ./docker/ ${server_user}@${PROD_IP}:/home/${server_user}/${app_name}
